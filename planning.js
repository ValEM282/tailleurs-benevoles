...
const nowGroups = document.getElementById("now-groups");
...
function updateFilterSummaries() {
  const postSummary = postFilter.querySelector("summary");
  const placeSummary = placeFilter.querySelector("summary");
  postSummary.textContent = selectedPostIds.size ? `${selectedPostIds.size} poste${selectedPostIds.size > 1 ? "s" : ""}` : "Tous les postes";
  placeSummary.textContent = selectedPlaceIds.size ? `${selectedPlaceIds.size} lieu${selectedPlaceIds.size > 1 ? "x" : ""}` : "Tous les lieux";
}
...
async function init() {
  ...
  if (!days.length) {
    showMessage("Aucun horaire n'est disponible pour tes responsabilités.");
    return;
  }

  const today = currentBrusselsParts().date;
